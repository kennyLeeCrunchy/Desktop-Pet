...
def settings_path() -> Path:
    ...
    folder = app_data / "KimachiDesktopPet"
...
def enforce_single_instance() -> None:
    ...
    handle = kernel32.CreateMutexW(None, False, "KIMACHI.DesktopPet.Singleton")
    if not handle:
        return
    if kernel32.GetLastError() == ERROR_ALREADY_EXISTS:
        ctypes.windll.user32.MessageBoxW(
            None,
            "KIMACHI is already running.",
            APP_NAME,
            0x40,
        )
        raise SystemExit(0)
...
class KimachiPet:
    ...
    def build_menu(self) -> None:
        ...
        self.menu.add_command(label="KIMACHI", state="disabled")
        ...
        self.menu.add_separator()
        self.menu.add_command(label="About KIMACHI", command=self.show_about)
    ...
    def show_about(self) -> None:
        messagebox.showinfo(
            APP_NAME,
            "KIMACHI\n\n"
            "Kim Chaewon's representative character.\n"
            "A brave, focused, and slightly tsundere golden baby cheetah.\n\n"
...
def main() -> None:
    ...
    app = KimachiPet()
    try:
        app.root.iconbitmap(resource_path("assets/kimachi.ico"))
    except tk.TclError:
        pass
    app.run()
